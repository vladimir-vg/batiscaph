...
handle_info(start_tracing, #shell_runner{} = State) ->
  ...
  % [E] = vision_probe_scenario:trace_started_events(erlang:system_time(micro_seconds), self()),
  vision_probe_scenario:trace_pid(self(), #{set_on_spawn => true}),
  % CollectorPid = whereis(vision_probe_collector),
...
setup(State) ->
  ScenarioPid = whereis(vision_probe_scenario),
  CollectorPid = whereis(vision_probe_collector),
  IoServerPid = whereis(vision_probe_io_server),
  ShellPid = self(),

  true = group_leader(IoServerPid, ShellPid),

  ok = gen_server:call(vision_probe_collector, {ignore_pids_tracing, [ScenarioPid, CollectorPid, IoServerPid, ShellPid]}),
...
restart_shell(#shell_runner{shell_pid = undefined} = State) ->
  ...
  ok = gen_server:call(vision_probe_collector, {ignore_pids_tracing, [Pid]}),
  vision_probe_collector ! {events, [E]},

  ok = gen_server:call(vision_probe_collector, {set_shell_pid, Pid}),
