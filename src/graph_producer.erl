...
% 
% 
% handle_call({websocket_subscribe, Pid}, _From, #delta_producer{websocket_pid = undefined} = State) ->
%   {reply, ok, State#delta_producer{websocket_pid = Pid}};
% 
...
% 
% 
% clear_fetch_timer(#delta_producer{fetch_timer = undefined} = State) ->
%   {ok, State};
% 
% clear_fetch_timer(#delta_producer{fetch_timer = Timer} = State) ->
%   erlang:cancel_timer(Timer),
%   {ok, State#delta_producer{fetch_timer = undefined}}.
% 
% 
% 
% set_fetch_timer(#delta_producer{fetch_timer = undefined} = State) ->
%   Timer = erlang:send_after(?FETCH_AFTER, self(), check_events),
%   {ok, State#delta_producer{fetch_timer = Timer}};
% 
% % timer is already set, do nothing
% set_fetch_timer(#delta_producer{fetch_timer = _} = State) ->
%   {ok, State}.
% 
% 
% 
% fetch_and_process_events(#delta_producer{id = Id} = State) ->
...
%   end.
% 
% fetch_events(#delta_producer{last_checked_at = LastAt, id = Id} = State) ->
...
%       {more, Events, State#delta_producer{last_checked_at = {At, Mcs}}};
% 
%     {ok, Events} ->
%       #{<<"at_s">> := At, <<"at_mcs">> := Mcs} = lists:last(Events),
%       {ok, Events, State#delta_producer{last_checked_at = {At, Mcs}}}
%   end.
% 
% 
% 
% send_delta_to_websocket(#delta_producer{last_delta_at = LastAt, id = Id, websocket_pid = Pid} = State) ->
%   {ok, Delta} = delta_json(Id, LastAt),
%   LastAt1 = lastest_timestamp_in_delta(LastAt, Delta),
% 
%   Pid ! {delta, Delta},
%   {ok, State#delta_producer{last_delta_at = LastAt1}}.
% 
% 
% 
